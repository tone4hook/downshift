export function parseRows(text:string) {
 ...
 text.split(/\r?\n/).forEach((line,index)=>{
  ...
  if(fields.length!==2||!sku||!/^\d+$/.test(raw!)||!Number.isSafeInteger(quantity)||quantity<1){errors.push({line:index,code:'INVALID_ROW'});return;}
  if(seen.has(sku)){errors.push({line:index,code:'DUPLICATE_SKU'});return;}

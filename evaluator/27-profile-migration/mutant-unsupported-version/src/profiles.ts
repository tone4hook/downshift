...
export function validateUrl(url:string) {let parsed;try{parsed=new URL(url);}catch{throw Error('INVALID_URL');}if(!['http:','https:'].includes(parsed.protocol))throw Error('INVALID_URL');return url;}
